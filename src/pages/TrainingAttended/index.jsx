  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        ...
        await logAudit('Updated', 'TrainingAttended', formData.record_id || formData.id || selectedId, 'Updated record details')
        toast.success('Training attended record updated successfully!')
      } else {
        const { data, error } = await supabase
          .from('training_attended')
          .insert([formData])
        ...
        await logAudit('Added', 'TrainingAttended', formData.record_id || data[0].record_id || data[0].id, 'Created new record')
        toast.success('Training attended record added successfully!')
      }
      setIsModalOpen(false)
    } catch (err) {
