-- =====================================================
-- FIX: CDRRMC Meeting saved_at Column
-- =====================================================
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_name = 'cdrrmc_meeting' AND column_name = 'saved_at'
  ) THEN
    ALTER TABLE cdrrmc_meeting ADD COLUMN saved_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();
...
BEGIN
  ...
  IF col_type != 'timestamp with time zone' THEN
    ...
    ALTER TABLE cdrrmc_meeting ADD COLUMN IF NOT EXISTS saved_at_backup TEXT;
    
    -- Copy data to backup
    UPDATE cdrrmc_meeting
    SET saved_at_backup = saved_at::text;
    
    -- Drop old column and create new one
    ALTER TABLE cdrrmc_meeting DROP COLUMN saved_at;
    ALTER TABLE cdrrmc_meeting ADD COLUMN saved_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();
    ...
    -- Drop backup column
    ALTER TABLE cdrrmc_meeting DROP COLUMN IF EXISTS saved_at_backup;
...
CREATE OR REPLACE FUNCTION update_cdrrmc_meeting_saved_at()
RETURNS TRIGGER AS $$
BEGIN
  NEW.saved_at = NOW();
  RETURN NEW;
...
DROP TRIGGER IF EXISTS set_cdrrmc_meeting_saved_at ON cdrrmc_meeting;

CREATE TRIGGER set_cdrrmc_meeting_saved_at
  BEFORE INSERT OR UPDATE ON cdrrmc_meeting
  FOR EACH ROW
  EXECUTE FUNCTION update_cdrrmc_meeting_saved_at();
...
SELECT 
  ...
  is_nullable
FROM information_schema.columns
WHERE table_name = 'cdrrmc_meeting'
...
WHERE event_object_table = 'cdrrmc_meeting'
  AND trigger_name = 'set_cdrrmc_meeting_saved_at';
...
BEGIN
  RAISE NOTICE '========================================';
  RAISE NOTICE 'CDRRMC Meeting saved_at column fixed!';
