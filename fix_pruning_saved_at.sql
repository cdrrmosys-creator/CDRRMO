-- =====================================================
-- FIX: Pruning & Trimming saved_at Column
-- =====================================================
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_name = 'pruning_trimming' AND column_name = 'saved_at'
  ) THEN
    ALTER TABLE pruning_trimming ADD COLUMN saved_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();
...
BEGIN
  ...
  IF col_type != 'timestamp with time zone' THEN
    ...
    ALTER TABLE pruning_trimming ADD COLUMN IF NOT EXISTS saved_at_backup TEXT;
    
    -- Copy data to backup
    UPDATE pruning_trimming
    SET saved_at_backup = saved_at::text;
    
    -- Drop old column and create new one
    ALTER TABLE pruning_trimming DROP COLUMN saved_at;
    ALTER TABLE pruning_trimming ADD COLUMN saved_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();
    ...
    -- Drop backup column
    ALTER TABLE pruning_trimming DROP COLUMN IF EXISTS saved_at_backup;
...
CREATE OR REPLACE FUNCTION update_pruning_trimming_saved_at()
RETURNS TRIGGER AS $$
BEGIN
  NEW.saved_at = NOW();
  RETURN NEW;
...
DROP TRIGGER IF EXISTS set_pruning_trimming_saved_at ON pruning_trimming;

CREATE TRIGGER set_pruning_trimming_saved_at
  BEFORE INSERT OR UPDATE ON pruning_trimming
  FOR EACH ROW
  EXECUTE FUNCTION update_pruning_trimming_saved_at();
...
SELECT 
  ...
  is_nullable
FROM information_schema.columns
WHERE table_name = 'pruning_trimming'
...
WHERE event_object_table = 'pruning_trimming'
  AND trigger_name = 'set_pruning_trimming_saved_at';
...
BEGIN
  RAISE NOTICE '========================================';
  RAISE NOTICE 'Pruning & Trimming saved_at column fixed!';
