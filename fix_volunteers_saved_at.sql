...
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE volunteers ADD COLUMN saved_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();
...
BEGIN
  ...
  IF col_type != 'timestamp with time zone' THEN
    ...
    ALTER TABLE volunteers ADD COLUMN IF NOT EXISTS saved_at_backup TEXT;
    ...
    ALTER TABLE volunteers DROP COLUMN saved_at;
    ALTER TABLE volunteers ADD COLUMN saved_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();
    ...
    -- Drop backup column
    ALTER TABLE volunteers DROP COLUMN IF EXISTS saved_at_backup;
...
CREATE OR REPLACE FUNCTION update_volunteers_saved_at()
RETURNS TRIGGER AS $$
BEGIN
  NEW.saved_at = NOW();
  RETURN NEW;
...
DROP TRIGGER IF EXISTS set_volunteers_saved_at ON volunteers;

CREATE TRIGGER set_volunteers_saved_at
  BEFORE INSERT OR UPDATE ON volunteers
  FOR EACH ROW
  EXECUTE FUNCTION update_volunteers_saved_at();
...
WHERE event_object_table = 'volunteers'
  AND trigger_name = 'set_volunteers_saved_at';
...
BEGIN
  RAISE NOTICE '========================================';
  RAISE NOTICE 'Volunteers saved_at column fixed!';
